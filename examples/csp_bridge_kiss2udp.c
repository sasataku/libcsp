...
#define DEFAULT_UART_NAME		"UART0"
...
void csp_input_hook(csp_iface_t * iface, csp_packet_t * packet) {
	...
	if (strncmp(iface->name, "UDP", strlen(iface->name)) == 0) {
		strncpy(dst_name, "UART", sizeof(dst_name));
	}
...
static void print_help(void) {
	csp_print("Usage: csp_bridge_kiss2udp [options]\n");
	csp_print(" --kiss                          set KISS interface\n");
...
static csp_iface_t * add_kiss_iface(const char * device_name)
{
	...
	int error = csp_usart_open_and_add_kiss_interface(&conf, DEFAULT_UART_NAME, 0, &iface);
...
int main(int argc, char * argv[]) {
	char default_uart_name[] = DEFAULT_UART_NAME;
	char * device_name = default_uart_name;
	...
	/* Add interfaces */
	kiss_iface = add_kiss_iface(device_name);
	udp_iface = add_udp_iface(address, lport, rport);

	/* Set interfaces to bridge */
	csp_bridge_set_interfaces(kiss_iface, udp_iface);
